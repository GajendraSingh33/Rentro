...
let AdminService = (() => {
    ...
    var AdminService = _classThis = class {
        constructor(userRepository, listingRepository, inquiryRepository, reviewRepository) {
        ...
        async listUsers(dto) {
            ...
            if (search) {
                queryBuilder.andWhere('(user.first_name ILIKE :search OR user.last_name ILIKE :search OR user.email ILIKE :search)', { search: `%${search}%` });
            }
            // Filter by role
            if (role) {
        ...
        async getPlatformOverview() {
            ...
            // User stats
            const [totalUsers, totalSeekers, totalOwners, totalAdmins, newUsersToday, newUsersThisWeek, newUsersThisMonth,] = await Promise.all([
                this.userRepository.count(),
                this.userRepository.count({ where: { role: 'SEEKER' } }),
                this.userRepository.count({ where: { role: 'OWNER' } }),
                this.userRepository.count({ where: { role: 'ADMIN' } }),
                this.userRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(todayStart) } }),
                this.userRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(weekStart) } }),
                this.userRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(monthStart) } }),
            ]);
            // Listing stats
            const [totalListings, activeListings, pendingListings, verifiedListings, featuredListings, newListingsToday, newListingsThisWeek,] = await Promise.all([
                ...
                this.listingRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(todayStart) } }),
                this.listingRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(weekStart) } }),
            ]);
            // Inquiry stats
            const [totalInquiries, pendingInquiries, respondedInquiries, newInquiriesToday] = await Promise.all([
                ...
                this.inquiryRepository.count({ where: { created_at: (0, typeorm_1.MoreThanOrEqual)(todayStart) } }),
            ]);
            // Review stats
        ...
        async getCityStats() {
            ...
            const citiesWithInquiries = await Promise.all(results.map(async (r) => {
